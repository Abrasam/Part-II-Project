...
if len(sys.argv) < 2:
    print("Required arguments: <number of clients> <node ip> <node port> <chunkx> <chunky>")
    sys.exit(-1)
...
chunkx,chunky=int(sys.argv[4]),int(sys.argv[5])
...
for i in range(num_clients):
    ...
    s.connect(addr)
    s.send(json.dumps({"type":"connect","player":f"{ii}","chunk":(chunkx,chunky)}).encode())
    ok = s.recv(2)
    if ok != b'ok':
        print("Error",file=sys.stderr)
    else:
        clients.append(Client(chunkx,chunky,ii,s))
while True:
    for c in clients:
        c.update()
    time.sleep(1/20.)
